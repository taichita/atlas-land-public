...
(mode,payload)=>{
  ...
  if(!window[key]){
    const state={documentId:crypto.randomUUID(),entries:new Map(),serial:0,auto:false,timer:null};
    window[key]=state;
  ...
  const known=new Map();
  for(const [id,entry] of state.entries){if(!entry.node.isConnected){state.entries.delete(id);continue;}known.set(entry.node,entry);}
  ...
  while((node=walker.nextNode())){
    ...
    const prior=known.get(node);if(prior?.translated===raw)continue;
