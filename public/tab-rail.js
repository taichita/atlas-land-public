export function setupTabRail(layout) {
  ...
  let timer,hovered=false,keyboard=false;
  ...
  rail.addEventListener('pointerleave',()=>{hovered=false;clearTimeout(timer);timer=setTimeout(()=>{if(!keyboard){if(rail.contains(document.activeElement))document.activeElement.blur();expand(false);}},220);});
  ...
  rail.addEventListener('focusout',()=>queueMicrotask(()=>{if(!hovered&&!rail.contains(document.activeElement))expand(false);}));
