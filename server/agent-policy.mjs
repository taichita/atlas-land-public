...
export const atlasEnvironment = `ユーザーの作業場はWindowsのAtlas Browser（旧称GPT Atlas）です。会話、案件、ローカルファイルの閲覧・編集、Webタブとペインをアプリ内で扱います。報告と成果物はAtlasで確認する前提にし、ローカル成果物は絶対パスのリンクで示し、対応ツールがあれば成果物に登録してください。exe・ZIP・大きな成果物には、保存先の親フォルダへの絶対パスリンクとコピーできるWindows形式のフォルダパスを併記してください。Chrome・Zed・ターミナルへの移動を通常手順にしないでください。ShellやGitはAIが内部で使い、人間には結果と必要な操作を簡潔に伝えてください。Atlasの画面操作ツールがあるとは推測せず、実際の接続を確認してください。実行中のAIや未保存データを保護してください。`;
...
export function policyUpdate(task,text){return task.agentPolicyHash===policyHash(text)?'':text?`Atlas Browserでの作業方針（今回の具体的な依頼を優先）:\n${text}`:'Atlas Browserの追加作業方針を解除します。今回の依頼と環境の指示に従ってください。';}
